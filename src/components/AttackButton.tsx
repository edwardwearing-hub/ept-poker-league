...
export default function AttackButton({ targetName, isTargetHijacked, onAttackSuccess }: Props) {
    ...
    return (
        <>
            ...
            <AnimatePresence>
                {showShareModal && (
                    <div className="fixed inset-0 z-[400] flex items-center justify-center p-4 bg-black/90">
                        <motion.div
                            initial={{ scale: 0.9, opacity: 0 }}
                            animate={{ scale: 1, opacity: 1 }}
                            className="bg-zinc-900 border-4 border-gold p-8 max-w-xl w-full text-center relative overflow-hidden"
                        >
                            <div className="absolute top-0 left-0 w-full h-2 bg-gradient-to-r from-red-500 via-gold to-red-500 animate-shimmer" />

                            <h2 className="text-3xl font-black text-white uppercase italic mb-2 tracking-tighter">Mission Successful</h2>
                            <p className="text-gold font-bold uppercase text-[10px] tracking-widest mb-6 italic animate-pulse">
                                Target Neutralized • Stats Compromised
                            </p>

                            {/* Preview Poster */}
                            <div className="relative aspect-[4/5] bg-charcoal border-4 border-zinc-800 mb-8 overflow-hidden group">
                                ...
                                <div className="absolute inset-0 flex flex-col items-center justify-center p-6">
                                    <div className="relative">
                                        ...
                                        <div className="absolute inset-0 flex items-center justify-center">
                                            <span className="text-5xl font-black text-ept-red uppercase -rotate-12 border-4 border-ept-red px-4 py-1">LOCKED</span>
                                        </div>
                                    </div>
                                    <div className="mt-4 text-center">
                                        <div className="text-2xl font-black text-white uppercase">{targetName}</div>
                            ...
                            <div className="grid grid-cols-2 gap-4">
                                <button
                                    ...
                                    className="flex items-center justify-center gap-2 bg-[#1877F2] hover:bg-[#166fe5] text-white py-3 font-bold uppercase text-xs transition-all"
                                >
                                    <Facebook className="w-4 h-4" /> Share to Facebook
                                </button>
                                <button
                                    onClick={copyLink}
                                    className="flex items-center justify-center gap-2 bg-white/10 hover:bg-white/20 text-white py-3 font-bold uppercase text-xs transition-all border border-white/10"
                                >
                            ...
                            <button
                                ...
                                className="mt-8 text-zinc-500 hover:text-white uppercase text-[10px] font-bold tracking-widest"
                            >
                                Return to Scouting
                            </button>
                        </motion.div>
